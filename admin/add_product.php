    ...
    if (empty($name) || empty($description) || empty($price)) {
      ...
      $image3 = handleFileUpload($_FILES['image3'], $targetDir);
        ...
        if($image === false || $image2 === false || $image3 === false) {
            ...
            $sql = "INSERT INTO products (name, description, price, image, image2, image3, added_date) VALUES (?, ?, ?, ?, ?, ?, NOW())";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ssdsss", $name, $description, $price, $image, $image2, $image3);
            if ($stmt->execute()) {
